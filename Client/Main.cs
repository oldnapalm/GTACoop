...
namespace GTACoOp
{
    public class Main : Script
    {
        ...
        public Main()
        {
            ...
            var selectedTraffic = trafficModes.IndexOf(PlayerSettings.TrafficMode.ToString());
            if (selectedTraffic < 0) selectedTraffic = 2;
            var trafficItem = new UIMenuListItem("Traffic Mode", trafficModes, selectedTraffic);
            trafficItem.OnListChanged += (item, index) =>
            {
                PlayerSettings.TrafficMode = (TrafficMode) Enum.Parse(typeof(TrafficMode), item.Items[index].ToString());
                Util.SaveSettings(null);
            };*/
        ...
        public void OnTick(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (PlayerSettings.TrafficMode)
                {
